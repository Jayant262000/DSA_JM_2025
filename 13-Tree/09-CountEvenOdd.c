/*
    Wrtie a C program to create BST and Count Odd and even elements in the BST
*/
...
void CountNodes(BST * Root,int * cntEve, int * cntOdd)
{
    if(Root!=NULL){
 
        (Root->data%2 == 0) ? (*cntEve) ++ : (*cntOdd) ++;
        CountNodes(Root->left,cntEve,cntOdd);
        CountNodes(Root->right,cntEve,cntOdd);
    }
...
int main()
{
    ...
    CountNodes(Root,&countEve,&countOdd);
	printf("\nTotal Number of Even Nodes are: %d\nTotal Number of Odd Nodes are: %d\n",countEve,countOdd);
